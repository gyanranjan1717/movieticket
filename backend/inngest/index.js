...
const releaseSeatsAndDeleteBooking = inngest.createFunction(
    ...
    async({event,step}) => {
        ...
        await step.run('check-payment-status', async () => {
            ...
            // If booking doesn't exist or is already paid, nothing to release
            if (!booking || booking.isPaid) {
                return;
            }

            // Atomic release of occupied seats
            if (booking.show && booking.bookedSeats?.length > 0) {
                const seatUnsets = {};
                booking.bookedSeats.forEach((seat) => {
                    seatUnsets[`occupiedSeats.${seat}`] = 1;
                });
                await Show.findByIdAndUpdate(booking.show, { $unset: seatUnsets });
